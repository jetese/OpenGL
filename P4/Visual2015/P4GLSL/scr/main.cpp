...
int main(int argc, char** argv)
{
	...
	initOGL();
	initShaderFw("../shaders_P4/fwRendering.v4.vert", "../shaders_P4/fwRendering.v4.frag");
	initShaderPP("../shaders_P4/postProcessing.v4.vert","../shaders_P4/postProcessing.v4.frag");
	initObj();
	initPlane();
	initFBO();
...
void renderFunc()
{
	...
	if (umaxdistance != -1) glUniform1fv(umaxdistance, 1, &maxDistanceFactor);
	if (ufocdistance != -1) glUniform1fv(ufocdistance, 1, &focalDistance);
...
void keyboardFunc(unsigned char key, int x, int y){
	...
	case 'n':
		...
		break;
	case 'a':
		maxDistanceFactor = maxDistanceFactor * 1 / 2;
